		...
		?>
			<form <?php echo $id . $class_names ?> role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				...
				<label>
					...
					<input type="search" class="search-field" placeholder="RECHERCHER"home" font_size="50px"]"<?php echo $attr_title ?>" value="<?php echo get_search_query() ?>" name="s" title=<?php echo $attr_title ?>" />
				</label>
				<?php
